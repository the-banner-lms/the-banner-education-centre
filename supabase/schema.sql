...
CREATE OR REPLACE FUNCTION public.next_tuition_invoice_number(p_academic_year INTEGER, p_prefix TEXT)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF NEW.invoice_number IS NULL THEN NEW.invoice_number := public.next_tuition_invoice_number(v_year, v_prefix); END IF;
