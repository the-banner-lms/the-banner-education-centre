...
BEGIN
  ...
  SELECT assigned_class INTO v_class_code FROM public.profiles WHERE id = NEW.student_id;
  ...
  IF NEW.invoice_number IS NULL THEN NEW.invoice_number := public.next_tuition_invoice_number(v_year, v_prefix, v_class_code); END IF;
