...
CREATE TABLE public.enrollment_submissions (
  ...
  student_number TEXT,
  payment_month TEXT,
  payment_method TEXT CHECK (payment_method IS NULL OR payment_method IN ('direct', 'kbzpay', 'wavepay', 'ayapay', 'bank_transfer')),
...
CREATE TABLE public.monthly_tuition_fees (
  ...
  base_status TEXT CHECK (base_status IS NULL OR base_status IN ('paid', 'unpaid', 'scholar')),
  yle_status TEXT CHECK (yle_status IS NULL OR yle_status IN ('paid', 'unpaid', 'scholar')),
