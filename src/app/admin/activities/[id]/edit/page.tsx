...
export default async function EditAdminAlbumPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-3xl mx-auto py-6 sm:px-6 lg:px-8">
      <div className="mb-6">
        <Link href={`/admin/activities/${album.id}`} className="text-blue-600 hover:text-blue-800 text-sm font-semibold mb-2 inline-block">
          &larr; Back to Album
        </Link>
        <h1 className="text-3xl font-bold text-gray-900">Edit Album: {album.title}</h1>
      </div>
      <EditAlbumForm album={album} basePath="/admin/activities" />
    </div>
  )
}
