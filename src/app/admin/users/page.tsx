...
export default async function AdminUsersPage() {
  ...
  // Group users by role
  const roles = ['admin', 'staff', 'teacher', 'student', 'editor']
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-8 text-gray-900">User Management</h1>
      ...
      {roles.map(role => {
        ...
        return (
          <div key={role} className="mb-10 bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-800">{roleLabels[role]}</h2>
              <span className={`px-3 py-1 rounded-full text-xs font-bold ${roleColors[role]}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap relative z-10">
                        <form action={async (formData) => {
                          ...
                          <select
                            ...
                            className={`text-xs rounded-full px-2 py-1 font-semibold border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 ${roleColors[user.role]}`}
                          >
